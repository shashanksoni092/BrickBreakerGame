		...
		for(int i=0;i<mg.map.length;i++) {
			for(int j=0;j<mg.map.length;j++) {
				if(mg.map[i][j]>0) {
					...
					Rectangle rect1=new Rectangle(ballposX,ballposY,20,20);
					Rectangle brickrect=rect;
					if(rect1.intersects(brickrect)) {
					...
					/*	if(ballposY+19<=brickY || ballposY+1>brickY) {
							ballYdir=-ballYdir;
						}*/
						else {
							ballYdir=-ballYdir;
